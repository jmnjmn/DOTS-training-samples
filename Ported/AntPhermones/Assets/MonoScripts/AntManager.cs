...
public class AntManager : MonoBehaviour
{
    ...
    [Range(0f, 1f)] public float antAccel;
    public float trailAddSpeed;
    [Range(0f, 1f)] public float trailDecay;
    ...
    void GenerateObstacles()
    {
        ...
        for (int i = 1; i <= obstacleRingCount; i++)
        {
            ...
            for (int j = 0; j < maxCount; j++)
            {
                ...
                if ((t * holeCount) % 1f < obstaclesPerRing)
                {
                    ...
                    obstacle.position = new Vector2(mapSize * .5f + Mathf.Cos(angle) * ringRadius,
        ...
        for (int i = 0; i < obstacleMatrices.Length; i++)
        {
            ...
            {
                obstacleMatrices[i][j] = Matrix4x4.TRS(output[i * instancesPerBatch + j].position / mapSize,
                    Quaternion.identity, new Vector3(obstacleRadius * 2f, obstacleRadius * 2f, 1f) / mapSize);
            }
        }
        ...
        for (int i = 0; i < obstacles.Length; i++)
        {
            ...
            for (int x = Mathf.FloorToInt((pos.x - radius) / mapSize * bucketResolution);
                x <= Mathf.FloorToInt((pos.x + radius) / mapSize * bucketResolution);
                x++)
            {
                ...
                for (int y = Mathf.FloorToInt((pos.y - radius) / mapSize * bucketResolution);
                    y <= Mathf.FloorToInt((pos.y + radius) / mapSize * bucketResolution);
                    y++)
                {
    ...
    void Start()
    {
        ...
        resourcePosition = Vector2.one * mapSize * .5f + new Vector2(Mathf.Cos(resourceAngle) * mapSize * .475f,
            Mathf.Sin(resourceAngle) * mapSize * .475f);
        resourceMatrix = Matrix4x4.TRS(resourcePosition / mapSize, Quaternion.identity,
            new Vector3(4f, 4f, .1f) / mapSize);
        ...
        {
            ants[i] = new Ant(new Vector2(Random.Range(-5f, 5f) + mapSize * .5f,
                Random.Range(-5f, 5f) + mapSize * .5f));
        }
    ...
    void FixedUpdate()
    {
        for (int i = 0; i < ants.Length; i++)
        {
            ...
            if (ant.holdingResource == false)
            {
                ...
                antColors[index1][index2] +=
                    ((Vector4) searchColor * ant.brightness - antColors[index1][index2]) * .05f;
            }
            else
            {
